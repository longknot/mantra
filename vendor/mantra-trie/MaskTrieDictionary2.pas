...
type
  ...
  public
    ...
    function AddAtState(const State: TStateHandle; const Value: TValue): Boolean;
...
begin
  ...
  begin
    ...
    if (State^.Flags and STATE_INITIALIZED) <> 0 then
      Exit;
...
begin
  ...
  State := FNodeManager.GetCurrentStateId;
...
begin
  ...
  if (Current^.Flags and TMaskNodeManager.STATE_HAS_VALUE) <> 0 then
    ...
    begin
      ...
      begin
        EmitContinue := True;
        EmitValue := PValue(Current^.Value)^;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FNodeManager.GetCurrentStateId = PrevState then
          Exit(False);
      end;
      State := FNodeManager.GetCurrentStateId;
      Exit(State <> TMaskNodeManager.INVALID_STATE_ID);
...
type
  TEdgeKind = (ekNone, ekSymbol, ekIndex);
var
  SavedState: TStateId;
  CurrentState: TStateId;
  ParentState: TStateId;
  RemovedRoot: TStateId;
  RemovedValues: SizeInt;
  Position: SizeInt;
  IndexStart: SizeInt;
...
begin
  ...
  ParentState := TMaskNodeManager.INVALID_STATE_ID;
  EdgeKind := ekNone;
  EdgeSymbol := 0;
  EdgeIndex0 := -1;
  RemovedRoot := TMaskNodeManager.INVALID_STATE_ID;
  CurrentState := FNodeManager.GetRootStateId;
  Position := 1;

  if not FNodeManager.SetCurrentStateId(CurrentState) then
    Exit(False);
  ...
  try
    ...
    begin
      ...
      begin
        Inc(Position);
        IndexStart := Position;
        while (Position <= Length(Path)) and (Path[Position] in ['0'..'9']) do
          Inc(Position);
        if (IndexStart = Position) or (Position > Length(Path)) or (Path[Position] <> ']') then
          Exit(False);

        IndexText := Copy(Path, IndexStart, Position - IndexStart);
        Index1Based := StrToIntDef(IndexText, 0);
        if Index1Based <= 0 then
          Exit(False);

        if not FNodeManager.TryGetIndexedChild(CurrentState, Index1Based - 1, CurrentState) then
          Exit(False);
        if not FNodeManager.SetCurrentStateId(CurrentState) then
          Exit(False);

        EdgeKind := ekIndex;
        EdgeIndex0 := Index1Based - 1;
        Inc(Position); // skip ']'
      end
      else
      begin
        if not FNodeManager.Advance(Byte(Path[Position])) then
          Exit(False);
        CurrentState := FNodeManager.GetCurrentStateId;
        EdgeKind := ekSymbol;
        EdgeSymbol := Byte(Path[Position]);
        Inc(Position);
      end;
    end;
    ...
    case EdgeKind of
      ekSymbol:
        begin
          if not FNodeManager.DetachSymbolChild(ParentState, EdgeSymbol, RemovedRoot) then
            Exit(False);
        end;
      ekIndex:
        begin
          if not FNodeManager.DetachIndexedChild(ParentState, EdgeIndex0, RemovedRoot) then
            Exit(False);
        end;
    else
      Exit(False);
    ...
    if RemovedRoot = TMaskNodeManager.INVALID_STATE_ID then
      Exit(False);

    RemovedValues := FNodeManager.ClearSubtreeValues(RemovedRoot);
    if RemovedValues > FCount then
      FCount := 0
    else
      Dec(FCount, RemovedValues);
    Result := True;
  finally
...
begin
  State := 0;
  SavedState := FNodeManager.GetCurrentStateId;
  if not FNodeManager.SetCurrentStateId(BaseState) then
    Exit(False);
  try
    for I := 1 to Length(Suffix) do
    begin
      if not FNodeManager.Advance(Byte(Suffix[I])) then
      begin
        PrevState := FNodeManager.GetCurrentStateId;
        FNodeManager.AddTransition(Byte(Suffix[I]));
        if FNodeManager.GetCurrentStateId = PrevState then
          Exit(False);
      end;
    end;
...
function TTrieDictionary.ResolveIndexedPattern(
  ...
  var
    PrefixKey: ansistring;
    StartState: TStateId;
    SegmentBuffer: ansistring;
    ...
    procedure Walk(const StateId: TStateId; var CurrentSegment: ansistring);
    var
      State: PMaskState;
      I: Integer;
      N: Integer;
      ...
      PreviousLength: SizeInt;
    begin
      State := FNodeManager.GetStateById(StateId);
      if State = nil then
        Exit;

      if (State^.Flags and TMaskNodeManager.STATE_HAS_VALUE) <> 0 then
        AddCandidate(CurrentSegment);
      if (State^.Flags and TMaskNodeManager.STATE_INITIALIZED) = 0 then
        Exit;
      if (State^.Flags and TMaskNodeManager.STATE_INDEXED) <> 0 then
        Exit;

      N := TMaskNodeManager.TransitionCount(State^.MaskBits);
      FixedBits := State^.SetBits and (not State^.MaskBits and $FF);
      for I := 0 to N - 1 do
      begin
        Child := FNodeManager.GetTransition(State, I);
        if Child = TMaskNodeManager.INVALID_STATE_ID then
          Continue;

        Symbol := Byte((TMaskNodeManager.UNPACK_BITS[State^.MaskBits, I] or FixedBits) and $FF);
        if AnsiChar(Symbol) = '.' then
        begin
          AddCandidate(CurrentSegment);
          Continue;
        end;

        PreviousLength := Length(CurrentSegment);
        SetLength(CurrentSegment, PreviousLength + 1);
        CurrentSegment[PreviousLength + 1] := AnsiChar(Symbol);
        Walk(Child, CurrentSegment);
        SetLength(CurrentSegment, PreviousLength);
      end;
    end;
  begin
    if Prefix = '' then
      StartState := FNodeManager.GetRootStateId
    else
    begin
      PrefixKey := Prefix + '.';
      if not TryLocatePrefixState(PrefixKey, StartState) then
        Exit;
    end;

    SegmentBuffer := '';
    Walk(StartState, SegmentBuffer);
  end;
